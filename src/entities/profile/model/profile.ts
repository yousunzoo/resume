...
export const aboutMe: string[] = [
  "프론트엔드를 화면을 구현하는 역할에 한정하지 않고, 비즈니스 요구사항을 안정적으로 변경하고 확장할 수 있는 구조로 만드는 일이라고 생각합니다. 누적 사용자 800만 명 규모의 금융 플랫폼을 3년간 개발·운영하며, 요구사항이 바뀌었을 때 수정 범위를 예측하고 기존 기능에 미치는 영향을 줄일 수 있는 구조를 만들어 왔습니다.",
  "복잡한 서비스에서는 화면 자체보다 상태와 정책이 어디에서 결정되고 어떻게 연결되는지가 중요하다고 봅니다. 신청·상담·관리자처럼 여러 단계와 역할이 연결되는 기능을 개발하며 상태와 비즈니스 로직의 책임을 분리하고, 변경과 문제 발생 시 원인을 추적하기 쉬운 구조로 개선해 왔습니다.",
