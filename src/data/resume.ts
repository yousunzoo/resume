...
export const projects: Project[] = [
  ...
  // ── 7. 클릭비 웹 제안서 자동 생성·발행 플랫폼 ───────────────────────────
  {
    slug: "clickb-automation",
    title: "클릭비 — 웹 제안서 자동 생성·발행 플랫폼",
    category: "백오피스·자동화",
    featured: true,
    period: "2026.08",
    role: "기획 · Frontend · Backend 전 과정 단독 개발",
    tech: [
      "Next.js 15",
      "React 19",
      "TypeScript",
    ...
    headline:
      "글만 준비하면 발주처에 보낼 웹 제안서가 나오는, 데이터 × 렌더러 분리 기반 제안서 자동 생성·발행 플랫폼",
    keyResult:
      "제안서 조판 과정 제거로 작성 시간 단축·품질 상향 평준화 · AI 없이도 완전 동작하는 폴백 구조 · PDF 대신 URL 발행",
    tags: ["Next.js", "AI 구조화", "데이터 × 렌더러", "풀스택"],
    summary:
      "영업 담당자가 제안서 원문만 붙여넣으면 시스템이 12개 섹션으로 구조화하고 고정 템플릿으로 렌더해 고유 URL로 발행하는 단일 Next.js 앱입니다. 기획·프론트엔드·백엔드 전 과정을 단독으로 설계·개발했습니다.",
    overview:
      "영업 건마다 제안서를 매번 새로 디자인·조판해 시간이 많이 들고 담당자·건별 품질 편차가 컸으며, 산출물이 PDF/PPT라 모바일 열람과 포트폴리오 연계가 불편했습니다. 발주 플랫폼 규정상 회사명·연락처 노출도 금지돼, '글만 준비하면 보낼 수 있는 웹 제안서가 나온다'를 컨셉으로 잡고 제안서를 데이터(내용) × 렌더러(디자인)로 분리해 템플릿화했습니다.",
    scope: [
      "기획 — 문제 정의, 사용 시나리오, 핵심 의사결정",
      "Frontend — 12개 섹션 렌더러, 편집 UI, 공개 제안서(스크롤스파이)",
      "Backend — route handler, 생성 파이프라인, AI 구조화, 데이터 모델",
      "공유 계약(packages/shared) — FE·BE 공용 섹션 스키마 단일 관리",
    ],
    problem:
      "영업 건마다 제안서를 새로 조판해 시간이 들고 품질 편차가 컸고, PDF/PPT는 모바일 열람과 포트폴리오 연계가 어려웠으며, 발주 플랫폼이 금지하는 회사명·연락처가 실수로 노출될 위험이 있었습니다.",
    decision:
      "PDF가 아니라 웹 링크로 발행하고, AI는 '생성'이 아니라 '구조화 + 가벼운 윤문'으로 한정했으며, AI 없이도 규칙 기반으로 완전 동작하도록 폴백을 설계했습니다.",
    decisions: [
      {
        heading: "PDF가 아닌 웹 링크(URL)로 발행",
        body: "결과물을 파일이 아니라 고유 URL(/p/[slug])로 만들어 모바일 열람·스크롤 네비·포트폴리오 연결을 자연스럽게 했습니다. 배포 후 편집해도 같은 링크에 그대로 반영돼 재전송이 필요 없습니다.",
      },
      {
        heading: "AI는 '생성'이 아니라 '구조화 + 가벼운 윤문'",
        body: "사실·수치·의미는 원문 그대로 고정하고(무손실), 형식이 제각각인 원문을 정해진 JSON 섹션으로 분해하되 서술형 산문의 어색한 표현만 다듬었습니다. 없는 내용은 지어내지 않도록 Structured Output(json_schema strict)으로 응답 모양을 강제했습니다.",
      },
      {
        heading: "AI 없이도 완전 동작하는 폴백",
        body: "AI는 선택 요소로 두고, 키 없음·빈 입력·거부·예외를 모두 null 반환으로 수렴시켜 결정론적 파서로 폴백했습니다. AI 장애에도 발행이 멈추지 않습니다.",
      },
      {
        heading: "데이터 × 렌더러 분리 + 공유 계약",
        body: "바뀌는 것은 데이터뿐, 레이아웃·디자인은 고정입니다. 12개 섹션마다 전용 렌더러가 해당 데이터만 받아 고정 디자인으로 그리고, FE·BE는 packages/shared의 섹션 스키마를 계약으로 주고받습니다. 신원 정보(회사명·연락처)는 기본값에서 구조적으로 배제했습니다.",
      },
    ],
    beforeAfter: [
      { aspect: "산출물", before: "PDF/PPT 파일", after: "고유 URL 웹 제안서" },
      { aspect: "제작 방식", before: "건별 수동 디자인·조판", after: "데이터 입력 → 자동 구조화·렌더" },
      { aspect: "수정·전달", before: "수정 후 파일 재전송", after: "같은 링크에 그대로 반영" },
      { aspect: "AI 의존", before: "—", after: "AI는 선택 · 규칙 기반 폴백으로 완전 동작" },
      { aspect: "신원 노출", before: "실수로 노출 위험", after: "비노출을 기본값으로 구조적 차단" },
    ],
    result:
      "원문 입력 → 구조화 → 편집 → 발행까지 전 과정이 동작하는 초안 개발을 완료했습니다(2026.08). 조판 과정을 없애 작성 시간을 줄이고 품질을 상향 평준화했으며, 링크 기반 전달로 열람 편의성과 포트폴리오 연계 설득력을 확보했습니다.",
    results: [
      "원문 입력 → 구조화 → 편집 → 발행 전 과정 동작 (초안 개발 완료)",
      "제안서 조판 과정 제거로 작성 시간 단축·품질 상향 평준화",
      "AI 구조화 + 규칙 기반 폴백으로 AI 없이도 완전 동작",
      "PDF 대신 URL 발행 — 모바일 열람·포트폴리오 연계·재전송 불필요",
    ],
    highlights: [
      "제안서를 데이터 × 렌더러로 분리해 12개 섹션 템플릿화",
      "OpenAI Structured Output(strict)으로 무손실 구조화 + 가벼운 윤문",
      "키 없음·거부·예외를 null로 수렴시켜 결정론 파서로 폴백",
      "FE·BE 공용 섹션 스키마(packages/shared)를 단일 계약으로 관리",
      "어댑터 계층(lib/api.ts)으로 admin 자동 연동 교체 지점 격리",
    ],
    interviewPoints: [
      "AI를 '생성'이 아니라 '구조화 + 윤문'으로 한정한 이유와 무손실 보장 방법",
      "AI 없이도 동작하도록 폴백 경계를 어디에 뒀는가",
      "데이터 × 렌더러 분리가 템플릿 유지보수에 주는 이점",
      "신원 비노출을 기본값으로 강제한 구조적 설계",
    ],
    relatedPosts: [
  ...
  // ── 8. 모여바 (mario-wifi-front) — 여행 바우처 셀프 관리 웹 ──────────────
  {
    slug: "moyeoba",
...
export const techStack: TechStackGroup[] = [
  ...
  {
    category: "State & Data",
    items: ["TanStack Query", "Zustand", "Jotai", "React Hook Form", "Zod"],
  },
  {
    category: "Testing",
