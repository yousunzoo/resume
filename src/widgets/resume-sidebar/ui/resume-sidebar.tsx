...
import { profile, techStack } from "@/entities/profile";
  ...
  return (
    ...
    >
      <div className="lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col lg:overflow-y-auto">
        <div className="flex h-full flex-col gap-10 px-7 py-9 lg:px-8 lg:pt-12 lg:pb-40 print:gap-6 print:px-0 print:pt-0 print:pb-6">
          {/* Identity — 인쇄에서는 사진(좌) + 이름/직함/태그라인(우) 가로 배치로
              상단 여백을 채우고 세로 높이를 줄인다. 화면은 기존 세로 스택 유지. */}
          <div className="print-block print:flex print:items-center print:gap-5">
            {profile.image && (
              <div className="relative mb-6 h-24 w-24 overflow-hidden rounded-full ring-1 ring-line-strong lg:h-28 lg:w-28 print:mb-0 print:h-20 print:w-20 print:shrink-0">
                <Image
                  src={profile.image}
            ...
            <div className="min-w-0 print:flex-1">
              ...
              <Text
                variant="body-sm"
                className="mt-5 border-t border-line pt-5 leading-relaxed text-ink-muted print:mt-2 print:border-t-0 print:pt-0"
              >
                {profile.tagline}
              </Text>
            </div>
          ...
          {/* Footer link */}
          <div className="no-print mt-auto border-t border-line py-5">
            <Link
              href={site.devlogUrl}
              target="_blank"
              rel="noopener noreferrer"
